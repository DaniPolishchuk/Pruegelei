...
export async function setFighterData(player, flip, fighterName) {
    try {
        ...
        const attackBoxOffsetX = flip ? baseAttackBoxOffset.x - attackBoxWidth : baseAttackBoxOffset.x;
        ...
        const createAttackBox = (attackKey) => {
            ...
            const offsetX = flip ? player.width - baseAttackBoxOffset.x - width : baseAttackBoxOffset.x;
            return {
                position: {x: player.position.x, y: player.position.y},
...
export async function setFighters(player1, player2) {
    ...
    getFighters().then(fighters => {
        ...
        player1.offset = {x: f1.BackgroundSelectionOffsetX, y: f1.BackgroundSelectionOffsetY};
        ...
        player2.offset = {x: f2.BackgroundSelectionOffsetX, y: f2.BackgroundSelectionOffsetY};
...
export function calculateCamera(player1, player2) {
    ...
    const playerRight = Math.max(player1.position.x + player1.width, player2.position.x + player2.width);
...
export function updateHorizontalMovement(player1, player2, leftPressed, rightPressed, leftKey, rightKey) {
    if (player1.isAttacking || player1.isBlocking || player1.dead || player1.currentSpriteName === "takeHit") return;
    let canMoveLeft = leftPressed && player1.position.x > 0;
    let canMoveRight = rightPressed && player1.position.x + player1.width < canvas.width;

    if (player1.hitbox.position.y + player1.hitbox.height > player2.hitbox.position.y) {
        if (player1.flip) {
            canMoveLeft = canMoveLeft && player1.hitbox.position.x > player2.hitbox.position.x + player2.hitbox.width;
        } else {
            canMoveRight = canMoveRight && player1.hitbox.position.x + player1.hitbox.width < player2.hitbox.position.x;
        }
    }
...
export function resolveVerticalCollisionBetweenFighters(player1, player2) {
    if (player1.velocity.y > 0) {
        if (
            player1.hitbox.position.x < player2.hitbox.position.x + player2.hitbox.width &&
            player1.hitbox.position.x + player1.hitbox.width > player2.hitbox.position.x
        ) {
            if (
                player1.hitbox.position.y + player1.hitbox.height > player2.hitbox.position.y &&
                player1.hitbox.position.y < player2.hitbox.position.y
            ) {
                player1.hitbox.position.y = player2.hitbox.position.y - player1.hitbox.height;
                player1.velocity.y = 0;

                const fighterCenter = player1.hitbox.position.x + player1.hitbox.width / 2;
                const otherCenter = player2.hitbox.position.x + player2.hitbox.width / 2;

                player1.velocity.x = fighterCenter < otherCenter ? -SLIP_SPEED : SLIP_SPEED;
            }
        }
    }
}
...
export function pollGamepadInputs(player1, player2) {
    ...
    const createGamepadState = (gp) => ({
        ...
        attack: false,
        attackStyle: null,
        block: gp.buttons[0].pressed || gp.buttons[1].pressed || gp.buttons[2].pressed || gp.buttons[3].pressed
    });
